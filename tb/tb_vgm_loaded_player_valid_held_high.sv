...
module tb_vgm_loaded_player_valid_held_high;
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            mem_rd_valid <= first_accept_seen;
    ...
    initial begin
        ...
        if (!header_valid || player_error) begin
            $display("FAIL held-high valid header_valid=%0b error=%0b magic=%08h",
                     header_valid, player_error, header_magic_read_debug);
            $finish;
        end
        if (header_magic_read_debug != 32'h206d6756) begin
            $display("FAIL held-high valid magic=%08h",
                     header_magic_read_debug);
            $finish;
        end

        $display("PASS tb_vgm_loaded_player_valid_held_high magic=%08h",
                 header_magic_read_debug);
        $finish;
    end
endmodule
