...
module tb_ym2610_gunfrontier_rg1_probe;
    ...
    task automatic check(input logic active, input logic [7:0] code,
                         ...
                         input logic current, input logic expected);
        begin
            fatal_active = active;
            reject_code = code;
            ...
            if (profile_fatal !== expected)
                $fatal(1, "RG1 mismatch code=%02h addr=%05h current=%0d got=%0d expected=%0d",
                       code, addr, current, profile_fatal, expected);
        end
    endtask

    initial begin
        check(1'b1, 8'h0b, 1'b1, 20'h07600, 1'b1, 1'b1);
        check(1'b1, 8'h0b, 1'b1, 20'h07601, 1'b1, 1'b0);
        check(1'b1, 8'h0b, 1'b1, 20'h07600, 1'b0, 1'b0);
        check(1'b1, 8'h0c, 1'b1, 20'h07600, 1'b1, 1'b0);
        check(1'b0, 8'h0b, 1'b1, 20'h07600, 1'b1, 1'b0);
        $display("PASS: YM2610 Gun Frontier RG1 probe");
        $finish;
    end
endmodule
