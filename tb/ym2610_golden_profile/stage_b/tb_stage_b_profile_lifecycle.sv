...
module tb_stage_b_profile_lifecycle;
    ...
    task automatic wait_state(input integer wanted, input integer limit);
        ...
        begin
            timeout = 0;
            while (profile.lifecycle_state != wanted && timeout < limit) begin
                @(posedge clk);
                timeout = timeout + 1;
            end
            if (profile.lifecycle_state != wanted)
    ...
    always_ff @(posedge clk) begin
        ...
        if (!reset) begin
            ...
            if (^{file_read_request, file_read_address, profile_status,
                  debug_page_data, scanner_start_count, audio_l, audio_r} === 1'bx)
    ...
    initial begin
        ...
        wait_state(7, 200000);
        if (scanner_start_count != 1 || profile.prepared_file)
            $fatal(1, "cold raw load result");

        begin_load("prepared_valid");
        wait_state(7, 200000);
        if (scanner_start_count != 2 || !profile.prepared_file ||
            profile.exact_original_size + 128 != uploaded_physical_size)
            $fatal(1, "prepared load/result");

        begin_load("b_only_key_on");
        wait_state(8, 200000);
        if (scanner_start_count != 3 || profile.scan_classification != 2 ||
            profile.scan_reject_code != 8'h03 || file_read_request)
        ...
        inject_stale = 1'b1;
        wait_state(8, 100);
        if (profile.profile_reject_code != 8'h0a || file_read_request)
        ...
        begin_load("standard_valid_raw");
        wait_state(7, 200000);
        if (profile.adapter_stale || scanner_start_count != 6)
        ...
        begin_load("standard_valid_raw");
        wait_state(7, 200000);
        if (scanner_start_count != 1 || profile.adapter_stale)
            $fatal(1, "reset-during-scan recovery");

        repeat (120000) @(posedge clk);
        if (profile.lifecycle_state != 7 || file_read_request ||
            profile.adapter_outstanding || scanner_start_count != 1)
        ...
        $display("PROFILE_LIFECYCLE_RESULT PASS starts=%0d accepts=%0d responses=%0d idle_equivalent_seconds=120",
                 scanner_start_count, accepts, responses);
        $finish;
    end
endmodule
