...
base_head="4aad88f30942ff8112f559b11129f136fd55b69b"
phase1b_subject="Add port 1 JT10 FM tone coverage"
phase1b_tmp="$(mktemp -d /private/tmp/jt10-phase1b.XXXXXX)"
trap 'rm -rf "$phase1b_tmp"' EXIT

work_root="$phase1b_tmp/work"
repeat_root="$phase1b_tmp/repeat"
...
run_phase1b() {
    ...
    ) > "$phase1b_tmp/$name.log"
    rg \
        '^(WARMUP_READY|FIRST_PUBLIC|PORT1_AUDIT|AUDIO_WINDOW|MUTE_SETTLE|KEYOFF_SETTLE|BUS_RESULT|SAMPLE_RESULT|TONE_RESULT|TONE_COMPARE|IDLE_RESULT|PHASE1B_RESULT|PHASE1B_PASS|FAIL)' \
        "$phase1b_tmp/$name.log"
}
...
normalize_phase1b() {
    local source_log="$1"
    rg \
        '^(BUS_WRITE|WARMUP_READY|FIRST_PUBLIC|PORT1_AUDIT|AUDIO_WINDOW|MUTE_SETTLE|KEYOFF_SETTLE|BUS_RESULT|SAMPLE_RESULT|TONE_RESULT|TONE_COMPARE|IDLE_RESULT|PHASE1B_RESULT)' \
        "$source_log" |
        sed -E 's/run=[0-9]+/run=N/'
}

echo "== Phase 1B baseline =="
...
artifact_digest > "$phase1b_tmp/repo-artifacts-before.sha256"
...
for fixture_file in \
    ...
    "$repo_root/tb/jt10_cpu_bus_bfm.sv" \
    "$repo_root/tb/tb_jt10_phase1a_fm_tone.sv" \
    "$repo_root/tb/run_jt10_phase1a.sh"; do
    [[ -f "$fixture_file" ]]
...
tree_digest "$work_root" > "$phase1b_tmp/work-tree.sha256"
tree_digest "$repeat_root" > "$phase1b_tmp/repeat-tree.sha256"
diff -u "$phase1b_tmp/work-tree.sha256" \
    "$phase1b_tmp/repeat-tree.sha256"
...
rg -Fq "{2'd0,3'd4}" \
    "$work_root/rtl/genesis_audio/jt12/standard_jt10/jt10_acc.v"
rg -Fq 'up_ch <= {part, selected_register[1:0]}' \
    "$work_root/rtl/genesis_audio/jt12/jt12_mmr.v"
rg -Fq 'up_keyon <= selected_register == REG_KON && !part' \
    "$work_root/rtl/genesis_audio/jt12/jt12_mmr.v"
rg -q 'busy lasts for 32 synth clock cycles' \
    "$work_root/rtl/genesis_audio/jt12/jt12_mmr.v"
echo "CHANNEL_CONTRACT port=1 register_offset=1 internal=5 keyon_port=0 keyon=15 audible=PASS"
...
while IFS= read -r source_file; do
    ...
    perl -ne \
        ...
        "$work_root/$source_file"
              ...
              "$phase1b_manifest") |
    sort > "$phase1b_tmp/modules.txt"
uniq -d "$phase1b_tmp/modules.txt" > "$phase1b_tmp/duplicates.txt"
[[ ! -s "$phase1b_tmp/duplicates.txt" ]]
...
(
    ...
    iverilog -g2012 -Wall -s tb_jt10_standalone_elaboration \
        -o "$phase1b_tmp/standalone.vvp" -f "$source_manifest"
    iverilog -g2012 -Wall -s tb_jt10_phase1a_fm_tone \
        -o "$phase1b_tmp/phase1a.vvp" \
        -f "$source_manifest" -f "$phase1a_manifest"
    iverilog -g2012 -Wall -s tb_jt10_phase1b_fm_tone \
        -o "$phase1b_tmp/phase1b.vvp" \
        -f "$source_manifest" -f "$phase1a_manifest" \
        -f "$phase1b_manifest"
) > "$phase1b_tmp/iverilog-nonsim.log" 2>&1
nonsim_warning_count="$(
    rg -c 'warning:' "$phase1b_tmp/iverilog-nonsim.log" || true
)"
echo "ICARUS_NON_SIM_WARNING_COUNT $nonsim_warning_count"
sed -n '/warning:/p' "$phase1b_tmp/iverilog-nonsim.log"
echo "ICARUS_NON_SIM_ELABORATION PASS"
echo "UNDEFINED_MODULES 0"
(
    cd "$work_root"
    vvp "$phase1b_tmp/standalone.vvp"
) | tee "$phase1b_tmp/standalone.log"

echo "== Phase 1A port-0 regression =="
(
    cd "$work_root"
    vvp "$phase1b_tmp/phase1a.vvp" +RUN_ID=1
) > "$phase1b_tmp/phase1a.log"
rg \
    '^(BUS_RESULT|SAMPLE_RESULT|TONE_RESULT|IDLE_RESULT|PHASE1A_RESULT|PHASE1A_PASS|FAIL)' \
    "$phase1b_tmp/phase1a.log"
rg -q \
...
rg -q \
    ...
    "$phase1b_tmp/phase1a.log"
rg -q '^PHASE1A_PASS run=1$' "$phase1b_tmp/phase1a.log"
echo "PHASE1A_REGRESSION PASS"

echo "== three-run Phase 1B determinism =="
for run_id in 1 2 3; do
    echo "-- RUN_ID=$run_id"
    run_phase1b "run-$run_id" "$phase1b_tmp/phase1b.vvp" "$run_id"
    rg -q \
    ...
    rg -q \
        ...
        "$phase1b_tmp/run-$run_id.log"
    normalize_phase1b "$phase1b_tmp/run-$run_id.log" \
        > "$phase1b_tmp/run-$run_id.normalized"
done
diff -u "$phase1b_tmp/run-1.normalized" \
    "$phase1b_tmp/run-2.normalized"
diff -u "$phase1b_tmp/run-1.normalized" \
    "$phase1b_tmp/run-3.normalized"
echo "HASH_AND_LANDMARK_REPEATABILITY runs=3 result=PASS"
...
(
    ...
    iverilog -g2012 -Wall -DSIMULATION \
        ...
        -o "$phase1b_tmp/phase1b-simulation.vvp" \
        -f "$source_manifest" -f "$phase1a_manifest" \
        -f "$phase1b_manifest"
) > "$phase1b_tmp/iverilog-simulation.log" 2>&1
sim_warning_count="$(
    rg -c 'warning:' "$phase1b_tmp/iverilog-simulation.log" || true
)"
echo "ICARUS_SIM_WARNING_COUNT $sim_warning_count"
sed -n '/warning:/p' "$phase1b_tmp/iverilog-simulation.log"
run_phase1b "simulation" "$phase1b_tmp/phase1b-simulation.vvp" 1
rg \
    '^(BUS_RESULT|SAMPLE_RESULT|TONE_RESULT|TONE_COMPARE|IDLE_RESULT|PHASE1B_RESULT)' \
    "$phase1b_tmp/run-1.log" > "$phase1b_tmp/nonsim-compare.txt"
rg \
    '^(BUS_RESULT|SAMPLE_RESULT|TONE_RESULT|TONE_COMPARE|IDLE_RESULT|PHASE1B_RESULT)' \
    "$phase1b_tmp/simulation.log" > "$phase1b_tmp/simulation-compare.txt"
diff -u "$phase1b_tmp/nonsim-compare.txt" \
    "$phase1b_tmp/simulation-compare.txt"
...
(
    ...
    verilator --lint-only --timing -Wall -Wno-fatal \
        --top-module tb_jt10_phase1b_fm_tone \
        -f "$source_manifest" -f "$phase1a_manifest" \
        -f "$phase1b_manifest"
) > "$phase1b_tmp/verilator.log" 2>&1
verilator_rc=$?
set -e
verilator_warning_count="$(
    rg -c '^%Warning-' "$phase1b_tmp/verilator.log" || true
)"
echo "VERILATOR_RC $verilator_rc"
echo "VERILATOR_WARNING_COUNT $verilator_warning_count"
sed -n 's/^%Warning-\([^:]*\).*/\1/p' \
    "$phase1b_tmp/verilator.log" | sort | uniq -c | sort -k2
if [[ "$verilator_rc" -ne 0 ]]; then
    tail -80 "$phase1b_tmp/verilator.log"
    exit "$verilator_rc"
fi
...
if rg -n \
    'standard_jt10|jt10_phase0_warmup_wrapper|jt10_phase1a|jt10_phase1b|jt10_cpu_bus_bfm|tb/jt10_pinned' \
...
artifact_digest > "$phase1b_tmp/repo-artifacts-after.sha256"
diff -u "$phase1b_tmp/repo-artifacts-before.sha256" \
    "$phase1b_tmp/repo-artifacts-after.sha256"
