...
int main(int argc,char** argv) try {
    ...
    auto bytes=readfile(argv[1]); bool good=std::string(argv[2])=="good";
    Vc2_sim_top d;
    ...
    for(int session=0;session<(good?2:1);session++) {
        ...
        uint64_t limit=good ? (expected_cycles*20000000/985248+1000000) : 2000000;
        for(;cycles<limit;cycles++) {
            ...
            if(rawv) {
                ...
                if(session==0) first_audio.push_back(a);
                    ...
                    require(pulses<first_audio.size() && a==first_audio[pulses],"repeat-session deterministic audio");
                }
            ...
            if(active) require(ce_count==((cycles-launch_at)*985248/20000000),"fractional CE cumulative count");
            ...
            if(pcm && session==0 && active) {
                ...
                if(audio_phase>=20000000) {
                    audio_phase-=20000000; int16_t s=(int32_t(d.audio<<14)>>14)>>2;
            ...
            if(d.done) {
                ...
                if(expected_cycles>100000) require(changes>100 && pulses>100,"no qualified tone audio");
                for(int i=0;i<1000;i++) { edge(); require(d.audio==0 && !d.reg_write,"EOF audio leak"); }
