...
int main(int argc,char** argv) try {
    ...
    bool prefix=mode=="prefix";
    unsigned ddr_delay=std::getenv("DDR_DELAY")?std::stoul(std::getenv("DDR_DELAY")):3;
    auto original=bytes;
    auto golden=std::getenv("GOLDEN_V1")?readfile(std::getenv("GOLDEN_V1")):bytes;
    auto alternate=std::getenv("ALTERNATE_V2")?readfile(std::getenv("ALTERNATE_V2")):bytes;
    ...
    if(good) for(size_t p=128;p<golden.size();p+=16) {
        if(golden[p]==0) expected_cycles+=le64(golden,p+8);
        if(golden[p]==1) expected.push_back({expected_cycles,golden[p+1],golden[p+2]});
    }
    std::vector<int32_t> first_audio;
    std::ofstream pcm; if(argc>3) pcm.open(argv[3],std::ios::binary);
    std::ofstream native_pcm;
    if(std::getenv("NATIVE_PCM")) native_pcm.open(std::getenv("NATIVE_PCM"),std::ios::binary);
    ...
    for(int session=0;session<(alternating?3:sequence?8:good&&!fault?2:1);session++) {
        bytes=(alternating && session==1)?alternate:original;
        if(sequence) {
        ...
        for(size_t j=0;j<(bytes.size()+7)/8;j++) {
            uint64_t word=0;for(unsigned k=0;k<8;k++) if(j*8+k<bytes.size()) word|=uint64_t(bytes[j*8+k])<<(8*k);
            memory.at(j)=word;
        }
        ...
        for(;cycles<limit;cycles++) {
            ...
            if(d.ddr_we || d.ddr_rd) {
                require(d.ddr_addr>=0x6000000 && d.ddr_addr<0x6180000,"DDR descriptor address range");
                unsigned a=d.ddr_addr-0x6000000;
                if(bytes[8]==2) require(!d.ddr_we,"v2 must never write expanded DDR records");
                ...
                if(d.ddr_we) { require(d.ddr_be==255,"descriptor byte enable"); memory.at(a)=d.ddr_din; }
                    ...
                    remaining=d.ddr_burst; read_address=a; delay=ddr_delay;
                }
            }
            ...
            if(rawv) {
                if(native_pcm && session==0) {
                    uint32_t bits=uint32_t(a);
                    char encoded[4]={char(bits),char(bits>>8),char(bits>>16),char(bits>>24)};
                    native_pcm.write(encoded,4);
                }
            ...
            if(prefix && active && d.native_cycle==20000) {
                std::cout<<"INTEGRATED PREFIX PLAYING writes="<<wi<<" native_cycle="<<d.native_cycle
                         <<" (full-file trace checked separately)\n";
                return 0;
            }
        }
