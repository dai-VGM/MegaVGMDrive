#!/usr/bin/env python3
"""Extract a small FM+PSG+PCM VGM excerpt for MiSTer mode5 BRAM testing."""
...
DEFAULT_OUTPUT = Path("/tmp/mister_vgm_local_mixed_excerpt.vgm")
DATA_OFFSET = 0x40
DEFAULT_TARGET_SAMPLES = 44_100 * 5
...
def collect_setup_writes(data: bytes, start: int, stop: int) -> tuple[bytearray, int, int, int]:
    pc = start
    setup = bytearray()
    ym0_count = 0
    ym1_count = 0
    ...
    return setup, ym0_count, ym1_count, psg_count
...
def main() -> None:
    ...
    setup, setup_ym0, setup_ym1, setup_psg = collect_setup_writes(source, start, first_e0_pc)
    window, pcm_excerpt, window_stats = collect_window(
        source, first_e0_pc, args.target_samples, source_pcm
    )
    if window_stats["pcm_seek"] == 0:
        raise ValueError("no 0xE0 PCM seek in selected window")
    if window_stats["dac_stream"] == 0:
    ...
    total_ym0 = setup_ym0 + window_stats["ym0"] + trailer_ym0
    total_ym1 = setup_ym1 + window_stats["ym1"]
    total_psg = setup_psg + window_stats["psg"]

    vgm = make_header(source, commands, window_stats["wait"])
    ...
    print(f"source_first_E0_seek={first_seek}")
    print(f"ym_port0_write_count={total_ym0}")
    print(f"ym_port1_write_count={total_ym1}")
    print(f"psg_write_count={total_psg}")
    print(f"pcm_seek_count={window_stats['pcm_seek']}")
    print(f"dac_stream_count={window_stats['dac_stream']}")
    print(f"total_wait_samples={window_stats['wait']}")
    print(f"duration_seconds={window_stats['wait'] / 44100:.3f}")
