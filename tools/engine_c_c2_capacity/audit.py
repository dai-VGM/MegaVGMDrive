...
BASE='e00601449d0e6d4b880578bb2b21d6cce92dbfb2'
ALLOWED=('rtl/engine_c_c2_capacity/','hw/engine_c_c2_capacity/',
         'tools/engine_c_c2_capacity/','docs/engine_c_c2_capacity/')
def run(*args): return subprocess.check_output(args,cwd=ROOT,text=True)
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--out',type=Path,default=Path('/tmp/megavgm-c2-capacity'))
    ...
    qsf=ROOT/'hw/engine_c_c2_capacity/MegaVGMPlayer_EngineC_C2_Capacity_MiSTer.qsf'
    ...
    for n in ('mvgmsid_loader','engine_c_lab','mister_vgm_md_top'):
        assert 'engine_c_c2_capacity/' in modules[n][0]
    for n in ('sid_native_scheduler','sid_session_wrapper'):
