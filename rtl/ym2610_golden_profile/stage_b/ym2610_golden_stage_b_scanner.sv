...
// Full-file compatibility scan only. No playback or sound-device output exists.
module ym2610_golden_stage_b_scanner #(
    ...
    output logic [31:0]           unsupported_opcodes,
    output logic [31:0]           ssg_writes,
    output logic [31:0]           adpcma_key_on_voices,
    output logic [31:0]           adpcma_key_off_voices,
    output logic [31:0]           adpcmb_start_count,
    output logic [31:0]           adpcmb_reset_count,
    ...
    localparam logic [3:0] VARIANT_STANDARD    = 4'd0;
    localparam logic [3:0] VARIANT_B_COMPAT    = 4'd1;
    localparam logic [3:0] VARIANT_B_REQUIRED  = 4'd2;
    localparam logic [3:0] VARIANT_DUAL        = 4'd3;
    localparam logic [3:0] VARIANT_MALFORMED   = 4'd4;
    localparam logic [3:0] VARIANT_UNSUPPORTED = 4'd5;
    ...
    logic [31:0] desc_a_source_pc [0:MAX_DESCRIPTORS-1];
    logic [31:0] desc_a_rom_size [0:MAX_DESCRIPTORS-1];
    logic desc_a_valid [0:MAX_DESCRIPTORS-1];
    ...
    logic [31:0] desc_b_source_pc [0:MAX_DESCRIPTORS-1];
    logic [31:0] desc_b_rom_size [0:MAX_DESCRIPTORS-1];
    logic desc_b_valid [0:MAX_DESCRIPTORS-1];
    ...
    function automatic [2:0] popcount6(input logic [5:0] value);
        integer bit_index;
        begin
            popcount6 = 3'd0;
            for (bit_index = 0; bit_index < 6; bit_index = bit_index + 1)
                popcount6 = popcount6 + value[bit_index];
        end
    endfunction
    ...
    wire [63:0] write_hash_next = hash_write(
        trace_hash, command_pc, total_samples, opcode, pending_port,
    ...
    always_comb begin
        mem_req = busy && fetch_state(state) && !read_pending &&
                  ((state == ST_HEADER) || (scan_pc < original_size));
        case (state)
    ...
    always_ff @(posedge clk) begin
        ...
        if (reset) begin
            ...
            busy <= 1'b0;
            accepted <= 1'b0;
            classification <= VARIANT_MALFORMED;
            ...
            command_count <= 32'd0;
            unsupported_opcodes <= 32'd0;
            ssg_writes <= 32'd0;
            adpcma_key_on_voices <= 32'd0;
            adpcma_key_off_voices <= 32'd0;
            adpcmb_start_count <= 32'd0;
            adpcmb_reset_count <= 32'd0;
            ...
            for (i = 0; i < MAX_DESCRIPTORS; i = i + 1) begin
                ...
                desc_a_file[i] <= '0;
                desc_a_source_pc[i] <= 32'd0;
                desc_a_rom_size[i] <= 32'd0;
                desc_a_valid[i] <= 1'b0;
                desc_b_logical[i] <= 20'd0;
                desc_b_length[i] <= 21'd0;
                desc_b_file[i] <= '0;
                desc_b_source_pc[i] <= 32'd0;
                desc_b_rom_size[i] <= 32'd0;
                desc_b_valid[i] <= 1'b0;
            end
        end else begin
            ...
            end else if (busy && fetch_state(state) && !read_pending &&
                         state != ST_HEADER && scan_pc >= original_size) begin
                classification <= VARIANT_MALFORMED;
                reject_code <= REJECT_RANGE;
                if (!first_bad_valid) begin
                    first_bad_valid <= 1'b1;
                    first_bad_pc <= scan_pc;
                    first_bad_data <= opcode;
                ...
                case (state)
                    ...
                    ST_VALIDATE: begin
                        ...
                        if ({header[3], header[2], header[1], header[0]} != 32'h206d6756 ||
                            header32(4) > 32'hffff_fffb ||
                            header32(4) + 32'd4 != original_size ||
                            original_size < 32'h80 ||
                            (header32(8) >= 32'h150 &&
                             header32(52) > 32'hffff_ffcb) ||
                            ((header32(8) < 32'h150 ? 32'h40 :
                              32'h34 + header32(52)) >= header32(4) + 32'd4) ||
                            ((header32(8) < 32'h150 ? 32'h40 :
                              32'h34 + header32(52)) < 32'h40) ||
                            ((header32(76) & 32'h3fff_ffff) == 0) ||
                            (header32(28) != 0 &&
                             (header32(28) > 32'hffff_ffe3 ||
                              (32'h1c + header32(28)) <
                              (header32(8) < 32'h150 ? 32'h40 :
                               32'h34 + header32(52)) ||
                              (32'h1c + header32(28)) >= original_size))) begin
                            classification <= VARIANT_MALFORMED;
                    ...
                    ST_COMMAND: if (mem_valid && read_pending) begin
                        ...
                        case (mem_data)
                            ...
                            default: begin
                                unsupported_opcodes <= unsupported_opcodes + 32'd1;
                                if (!first_bad_valid) begin
                                    first_bad_valid <= 1'b1;
                                    first_bad_pc <= scan_pc;
                                ...
                                classification <= VARIANT_UNSUPPORTED;
                    ...
                    ST_WRITE_DATA: if (mem_valid && read_pending) begin
                        ...
                        trace_hash <= write_hash_next;
                        if (total_writes < 32'd256)
                            first256_trace_hash <= write_hash_next;
                        if (compat_semantic == 4'd1)
                            ssg_writes <= ssg_writes + 32'd1;
                        if (pending_port && pending_address == 8'h00) begin
                            if (mem_data[7])
                                adpcma_key_off_voices <=
                                    adpcma_key_off_voices + popcount6(mem_data[5:0]);
                            else
                                adpcma_key_on_voices <=
                                    adpcma_key_on_voices + popcount6(mem_data[5:0]);
                        end
                        if (!pending_port && pending_address == 8'h10) begin
                            if (mem_data[7])
                                adpcmb_start_count <= adpcmb_start_count + 32'd1;
                            if (mem_data[0])
                                adpcmb_reset_count <= adpcmb_reset_count + 32'd1;
                        end
                                ...
                                if ((block_type == 8'h82 &&
                                    ...
                                    ({mem_data, block_logical_start[23:0]} >
                                     ((block_type == 8'h82 ? 32'h0010_0000 :
                                                             32'h0008_0000) -
                                      (block_size - 32'd8))) ||
                                    (command_pc + 32'd7 > original_size) ||
                                    (block_size >
                                     original_size - (command_pc + 32'd7))) begin
                                    reject_code <= REJECT_RANGE;
                                    state <= ST_FATAL;
                                    ...
                                    if (block_type == 8'h82) begin
                                        ...
                                        desc_a_source_pc[descriptor_a_count[2:0]] <=
                                            command_pc;
                                        desc_a_rom_size[descriptor_a_count[2:0]] <=
                                            block_rom_size;
                                        desc_a_valid[descriptor_a_count[2:0]] <= 1'b1;
                                        ...
                                        desc_b_file[descriptor_b_count[2:0]] <=
                                            command_pc[ADDR_WIDTH-1:0] + 23'd15;
                                        desc_b_source_pc[descriptor_b_count[2:0]] <=
                                            command_pc;
                                        desc_b_rom_size[descriptor_b_count[2:0]] <=
                                            block_rom_size;
                                        desc_b_valid[descriptor_b_count[2:0]] <= 1'b1;
                    ...
                    ST_FINISH: begin
                        ...
                        if (loop_target != 0 && !loop_boundary_seen) begin
                            ...
                            accepted <= 1'b0;
                            classification <= first_bad_b_only ?
                                VARIANT_B_REQUIRED : VARIANT_MALFORMED;
