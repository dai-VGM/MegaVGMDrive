...
always @(posedge clk) begin
    if (!rst && debug_comb_x_count < 20 &&
        (((^snd_in) === 1'bx) || ((^prev) === 1'bx) ||
         ((^snd_out) === 1'bx))) begin
        debug_comb_x_count = debug_comb_x_count + 1;
        $display("COMB_X %m count=%0d time=%0t cen=%0b snd_in=%0d prev=%0d mem0=%0d snd_out=%0d",
                 debug_comb_x_count, $time, cen, snd_in, prev, mem[0], snd_out);
    end
end
`endif
