...
always @(posedge clk or negedge rst_n)
    if(!rst_n) begin
        ...
        // Match the source-defined inactive phase (cnt=0, adv=1) at every
        // accepted START, independent of the cen55 phase at the CPU write.
        cnt <= 'd0;
        adv <= 'b1;
...
always @(posedge clk or negedge rst_n)
    if(!rst_n) begin
        ...
        last_set <= set_flag;
        if( clr_flag || (acmd_up_b && on) )
            flag <= 1'b0;
        else if( !last_set && set_flag )
            flag <= 1'b1;
    end
