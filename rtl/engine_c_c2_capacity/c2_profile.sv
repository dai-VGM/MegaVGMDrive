...
// Minimal Golden Shell v1.1 audio ABI adapter, NOT production transport v1.3.
module golden_player_shell_v1_1_profile #(
    parameter integer VGM_ADDR_WIDTH=23, CLK_SYS_HZ=20000000
)(
    input logic clk_sys, reset, download_active,
    ...
    logic claimed, start, loaded, ready;
    ...
    engine_c_lab #(.AW(VGM_ADDR_WIDTH),.SYS_HZ(CLK_SYS_HZ)) engine (
        .clk(clk_sys),.reset(session_reset),.start(start),.file_size(uploaded_physical_size),
        .rd_req(file_read_request),.rd_addr(file_read_address),.rd_ready(file_read_ready),
        .rd_valid(file_read_valid),.rd_data(file_read_data),.busy(playback_active),
        .done(profile_done),.fatal(profile_fatal),.loaded(loaded),.error_code(error_code),
