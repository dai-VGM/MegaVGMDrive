...
module mvgmsid_loader #(
    ...
    wire [31:0] clock_num=header[160+:32], clock_den=header[192+:32];
    wire [7:0] model=header[224+:8], timing=header[232+:8];
    // Frozen C1 clocks, including strictly equivalent rational encodings.
    // 32-bit denominator * 20-bit Hz fits 52 bits; never multiply at 32 bits.
    wire clock_ok=clock_num!=0 && clock_den!=0 &&
        ((timing==1 && {32'd0,clock_num}==64'd985248*{32'd0,clock_den}) ||
         (timing==2 && {32'd0,clock_num}==64'd1022727*{32'd0,clock_den}));
    ...
    always @(posedge clk) begin
        if(reset) begin
            ...
            HEADER: begin
                ...
                // v1 admitted C3 subset: validated PAL/NTSC, 6581/8580, single.
                else if(!clock_ok || (model!=1 && model!=2) ||
            ...
            FINISH: if(!loaded) begin
                // Commit configuration once, only after complete-file validation.
                // SID remains in reset until descriptor store preparation ends.
                session_model<=(model==2);session_timing<=timing;
                session_clock_num<=clock_num;session_clock_den<=clock_den;
                loaded<=1;
            end
            default: ;
            endcase
