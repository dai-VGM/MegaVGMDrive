...
module sid_native_scheduler #(
    parameter integer SYS_HZ=20000000, SID_HZ=985248, RW=12
)(
    ...
    logic running;
    logic [31:0] phase;
    wire [32:0] sum = {1'b0,phase}+SID_HZ;
    typedef enum logic[1:0] {FETCH, RECEIVE, HAVE} fetch_t;
    fetch_t fetch;
    logic [77:0] head;
    wire [63:0] due=head[76:13];
    wire launch=enable && !launched && fetch==HAVE;
    // Free running after launch, INCLUDING EOF/fatal. Never pause to catch up.
    assign ce_sid=!reset && running && sum>=33'(SYS_HZ);
    wire tick=launch || ce_sid;
    wire [63:0] target=launch ? 64'd0 : native_cycle+1;
    assign mem_req=enable && fetch==FETCH && !done && !fatal;
    assign reg_write=!reset && !done && !fatal && tick && fetch==HAVE &&
                     !head[77] && due==target;
    assign reg_addr=head[12:8];
    assign reg_data=head[7:0];
    assign busy=launched && !done && !fatal;
    always @(posedge clk) begin
        if(reset) begin
            running<=0; phase<=0; fetch<=FETCH; mem_addr<=0; head<=0;
            done<=0; fatal<=0; native_cycle<=0; launched<=0;
        end else begin
            if(running) phase<=ce_sid ? 32'(sum-33'(SYS_HZ)) : sum[31:0];
            if(mem_req) fetch<=RECEIVE;
            if(fetch==RECEIVE && mem_valid) begin head<=mem_data; fetch<=HAVE; end
            if(launch) begin running<=1; launched<=1; phase<=0; end
            if(ce_sid && !done && !fatal) begin
