...
always @(posedge clock) begin
	...
	if(reset) begin
		oscillator <= 0; osc_msb_in_prv <= 0; test_delay <= 0;
...
reg [11:0] norm_dac;
always @(posedge clock) norm_dac <= reset ? 12'd0 : (mode ? norm : norm_6581);
...
// for OSC3 readback
reg [7:0] wave_out;
always @(posedge clock) if(reset) wave_out <= 0; else if(ce_1m) wave_out <= norm[11:4] | comb;
...
always @(posedge clock) begin
	...
	if(reset) begin
		keep_cnt <= 0; env_dac <= 0; dac_out <= 0; dca_out <= 0;
	end else if(ce_1m) begin
