...
generate
	...
	for(i=0; i<N; i=i+1) begin :chip
		...
		always @(posedge clk) Fc_offset[i] <= reset ? 13'd0 : (i ? fc_offset_r : fc_offset_l);
...
always @(posedge clk) begin
	...
	if(reset) begin
		...
		f_acc_t <= 0;
		v <= 0;
		for(integer k=0; k<N*3; k=k+1) begin
			_st_out[k] <= 0; p_t_out[k] <= 0;
			ps__out[k] <= 0; pst_out[k] <= 0;
		end
	end else begin
	...
	case(state)
		1,3,5,7,9,11: if(DUAL || state < 7) begin
...
always @(posedge clk) begin
	...
	if(reset) begin
		audio0 <= 0;
		for(integer k=0; k<N; k=k+1) audio[k] <= 0;
		publish_pending <= 0;
		sample_pulse <= 0;
	end else begin
		sample_pulse <= 0;
		if(ce_1m) publish_pending <= 1;
