...
always @(posedge clock) begin
	if(reset) p_t_out <= 0;
	else p_t_out <= mode ? wave8580_p_t[acc_t[10:0]] : wave6581_p_t[acc_t[10:0]];
end

// P + S
always @(posedge clock) begin
	if(reset) ps__out <= 0;
	else ps__out <= mode ? wave8580_ps_[acc_t] : wave6581_ps_[acc_t[10:0]];
end
...
generate
	if(MULTI_FILTERS) begin
		...
		always @(posedge clock) if(reset) f0 <= 0; else f0 <= f6581_curve[{cfg, Fc[10:1]}];
	end
	else begin
		always @(posedge ld_clk) if(ld_wr) f6581_curve[ld_addr[9:0]] <= ld_data;
		always @(posedge clock) if(reset) f0 <= 0; else f0 <= f6581_curve[Fc[10:1]];
	end
endgenerate
...
reg [15:0] f0_adj;
always @(posedge clock) if(reset) f0_adj <= 0; else f0_adj <= {fc_x[10], f6581_adj[tanh_x_mirror(fc_x)]};
