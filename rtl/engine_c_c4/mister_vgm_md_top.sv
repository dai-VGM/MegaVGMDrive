...
//
// Engine C C4 common-transport adapter, dedicated project only.
// Retains the C2-capacity emu port/parameter shape (legacy debug ports included)
// and physical DDR path. Adds the exact production B compatibility publisher
// and A/B single fade owner. The SID profile exposes parser EOF separately from
// transport END; final post-mix gain is owned only by megavgm_transport_owner.
...
`endif
    ...
    input  logic [127:0]      transport_hps_status,
    output wire [127:0]       transport_status_in,
    output wire               transport_status_set,
    ...
    wire ua_busy,ua_valid,ua_rd,ua_we,st_busy,st_valid,st_rd,st_we;
    wire [28:0] ua_addr,st_addr;
    wire [63:0] ua_din,ua_dout,st_din,st_dout;
    wire [7:0] ua_be,ua_burst,st_be,st_burst;
    c2_ddr_mux descriptor_upload_mux(
        .clk(clk),.reset(reset),
        .a_rd(ua_rd),.a_we(ua_we),.a_addr(ua_addr),.a_din(ua_din),.a_be(ua_be),.a_burst(ua_burst),
        .a_busy(ua_busy),.a_valid(ua_valid),.a_dout(ua_dout),
        .b_rd(st_rd),.b_we(st_we),.b_addr(st_addr),.b_din(st_din),.b_be(st_be),.b_burst(st_burst),
        .b_busy(st_busy),.b_valid(st_valid),.b_dout(st_dout),
        .busy(ddram_busy),.valid(ddram_dout_ready),.dout(ddram_dout),
        .rd(ddram_rd),.we(ddram_we),.addr(ddram_addr),.din(ddram_din),.be(ddram_be),.burst(ddram_burstcnt)
    );

    // Frozen production owner + publisher, adapted to the C3 profile facts.
    wire admitted_download, admitted_write, backend_wait, transport_vgm_download;
    wire io_fault, physical_io_fault;
    logic raw_return_d, raw_response_fault;
    // Stable backend emits mem_rd_valid one clock after a real DDR return.
    // Its other two producers synthesize VGM EOF on timeout/out-of-range;
    // neither is valid MVGMSID data, even if 0x66 is a legal WRITE value.
    wire fabricated_raw_response=profile_file_read_valid && !raw_return_d;
    always @(posedge clk) begin
        if(reset) begin raw_return_d<=0;raw_response_fault<=0;end
        else begin
            raw_return_d<=ua_valid;
            if(transport_load_begin) raw_response_fault<=0;
            else if(fabricated_raw_response) raw_response_fault<=1;
        end
    end
    assign io_fault=physical_io_fault || raw_response_fault || fabricated_raw_response;
    c4_ddr_health #(.BUSY_CYCLES(CLK_SYS_HZ),.DESCRIPTOR_CYCLES(CLK_SYS_HZ)) ddr_health(
        .clk(clk),.reset(reset),.new_session(transport_load_begin),
        .work_pending(upload_load_busy || transport_vgm_download || transport_state==1 || transport_state==2),
        .busy(ddram_busy),.rd(ddram_rd),.valid(ddram_dout_ready),.burst(ddram_burstcnt),.address(ddram_addr),
        .fatal(physical_io_fault)
    );
    wire transport_load_begin, transport_load_complete, transport_load_accepted;
    wire transport_session_start, transport_playback_start, transport_done;
    wire [31:0] transport_session, transport_done_session, transport_error_session;
    wire [2:0] transport_state;
    wire [7:0] published_error;
    ...
    wire transition_audio_open=profile_audio_enable && profile_ready &&
        !reset && !transport_vgm_download && !upload_load_error && !upload_load_overflow && !io_fault;
    megavgm_transport_owner #(.MODE5_TRACK_FADE_CYCLES(CLK_SYS_HZ/10)) transition_owner(
        .clk(clk),.reset(reset),
        .ioctl_download(ioctl_download),.ioctl_wr(ioctl_wr),.ioctl_index(ioctl_index),
        .ioctl_addr(ioctl_addr),.ioctl_dout(ioctl_dout),
        // On explicit I/O failure discard remaining host bytes and release
        // backpressure so Main can finish its transaction and observe FATAL.
        .mode5_backend_ioctl_wait(backend_wait && !io_fault),
        .playback_started(profile_started),.player_busy(player_busy),
        .loaded_player_done(parser_done),.session_id(transport_session),
    ...
    golden_shell_transport transport(
        .clk(clk),.reset(reset),
        .download(ioctl_index==16'd2 ? ioctl_download : admitted_download),
        .write_byte(admitted_write),.index(ioctl_index),.address(ioctl_addr),.data(ioctl_dout),
        .upload_busy(upload_load_busy),.upload_done(upload_load_done),
        ...
        .load_begin(transport_load_begin),.load_accepted(transport_load_accepted),
        .load_complete(transport_load_complete),.session_start(transport_session_start),
        .playback_start(transport_playback_start),.session_id(transport_session),
        .state(transport_state),.done(transport_done),.done_session_id(transport_done_session),
        .error_session_id(transport_error_session),.error_code(published_error),
        .loop_valid(transport_loop_valid),.loop_count(transport_loop_count)
    );
    logic [31:0] begin_count,complete_count,end_count,start_count;
    logic transport_done_d;
    always @(posedge clk) begin
        if(reset) begin
            begin_count<=0;complete_count<=0;end_count<=0;start_count<=0;transport_done_d<=0;
        end else begin
            transport_done_d<=transport_done;
            if(transport_load_begin) begin_count<=begin_count+1;
            if(transport_load_complete) complete_count<=complete_count+1;
            if(transport_done && !transport_done_d) end_count<=end_count+1;
            if(transport_playback_start) start_count<=start_count+1;
        end
    end

    engine_c_transport_profile #(
    ...
    golden_player_shell_upload #(
        ...
        .clk(clk),
        .reset(reset),
        .ioctl_download(transport_vgm_download),
        .ioctl_wr(admitted_write && !io_fault),
        .ioctl_addr(ioctl_addr),
        .ioctl_dout(ioctl_dout),
        .ioctl_index(ioctl_index),
        .ioctl_wait(backend_wait),
        .file_read_request(profile_file_read_request),
        .file_read_address(profile_file_read_address),
        .file_read_ready(profile_file_read_ready),
        .file_read_valid(profile_file_read_valid),
        ...
        .ddram_busy(ua_busy),
        .ddram_burstcnt(ua_burst),
        .ddram_addr(ua_addr),
        .ddram_dout(ua_dout),
        .ddram_dout_ready(ua_valid),
        .ddram_rd(ua_rd),
        .ddram_din(ua_din),
        .ddram_be(ua_be),
        .ddram_we(ua_we)
    ...
    // Identical final-mix gain arithmetic to the B production adapter.
    wire signed [25:0] scaled_l=$signed(profile_audio_l)*$signed({1'b0,transition_gain});
    wire signed [25:0] scaled_r=$signed(profile_audio_r)*$signed({1'b0,transition_gain});
    assign audio_l=transition_audio_open ? 16'(scaled_l >>> 8) : 16'sd0;
    assign audio_r=transition_audio_open ? 16'(scaled_r >>> 8) : 16'sd0;
    assign audio_sample_valid=transition_audio_open && profile_audio_sample_valid;
    assign player_done=transport_done;
    assign startup_reset_active = 1'b0;
    assign startup_waiting = 1'b0;
    assign startup_done = 1'b1;
    assign audio_gate_open = transition_audio_open;
    assign audio_muted = !transition_audio_open;
...
`else
    assign vgm_player_error_code = profile_status[15:8];
`endif
    assign vgm_error_pc_debug = '0;
    assign vgm_error_cmd_debug = '0;
    assign vgm_error_session_id = transport_error_session;
...
`endif
    ...
    assign mode5_load_begin_count = begin_count;
    assign mode5_load_done_edge_count = complete_count;
    assign mode5_sound_reset_start_count = begin_count;
    assign mode5_player_start_count = start_count;
    assign mode5_player_reset_count = begin_count;
    assign mode5_playback_session_id = transport_session;
    assign mode5_duplicate_start_blocked_count = '0;
    assign mode5_player_end_count = end_count;
    assign mode5_repeat_restart_count = '0;
    assign mode5_done_armed_debug = profile_started;
    assign mode5_repeat_session_id = '0;
    assign mode5_done_session_id = transport_done_session;
